...
<div id="info">
    ...
    if (isset ( $_POST['addInvoice'] )) {
        echo ("<sript>alert('hi there');</script>");
        $insertInvoice = myDB::getInstance ()->insert ( "transwise_invoice", array (
            'invoice_id' => 'invoice-000002',
            'booking_id' => 'booking-000001',
            'duty_slip_id' => 'DS-000001',
            'timestamp' =>time()
        ) );
        if ($insertInvoice) {
            echo "<div class='alert alert-success'> Package has been successfully created.</div>";
        } else {
            echo "<div class='alert alert-danger'> Internal error occured. Something went wrong. </div>";
        }
    }
    ...
    if (isset ( $_request ['delete'] )) {
        ...
        <div class="bs-callout bs-callout-warning">
            Are you sure you want to delete invoice <b><?php echo "$data[name]"; ?></b>
            ? <br>
            ...
            <form class="form-inline" role="form"
                  action='/operator/generateinvoice?all ' method='post'
                ...
                <a class="btn btn-default" href="./operator/generateinvoice?all"
...
if (isset ( $_request ['add'] ) || isset ( $_request ['edit'] )) {
    ...
    <div class="form-group">
        ...
        <div class="col-sm-5">
            <textarea class="form-control middle" tabindex="4" rows="3"
                      cols="7" placeholder="Address" name='address' required><?php //echo $companyEdit != null ? $companyEdit->address : ''; ?> </textarea>
        </div>
    </div>
        ...
        <td colspan="2">
            <div class="row omb_row-sm-offset-3 omb_loginOr">
                <div class="col-xs-12 col-sm-12">
                    <hr class="omb_hrOr">
                    <span class="omb_spanOr">Select Primary Duty Slip </span>
                </div>
    ...
    <tr>
        <td width="50%">
            <div class="form-group ">
                <label for="inputEmail3" class="col-sm-5 control-label">Company
                    Name</label>
        ...
        <td width="50%">
            <div class="driverinfo">
                <div class="form-group ">
                    <label for="inputEmail3" class="col-sm-5 control-label">Duty slip </label>

                    <div class="col-sm-7">
                        <select class="form-control car" name="car_id[]" required>
                            <option value="x">Vehicle Model Group</option>
                            <?php
                            if (isset ( $_request ['edit'] )) {
                                $groupinfo = new Cars ();
    ...
    // if (isset($_request['edit']))
    // {
    // echo ($data['enable_upto'] == '1') ? 'checked' : '';
    // }		?>
    <!-->
    <!--                    Upto : </label>-->
    <!---->
    ...
    // if (isset($_request['edit']))
    // {
    // echo ($data['enable_upto_hr'] == '1') ? 'checked' : '';
    // }		?>
    <!-->
    <!--                    Upto : </label>-->
    <!---->
    ...
    <tr>
        <td colspan="2">
            <div class="row omb_row-sm-offset-3 omb_loginOr">
                <div class="col-xs-12 col-sm-12">
                    <hr class="omb_hrOr">
                    <span class="omb_spanOr">Add More Duty Slips</span>
                </div>
                <input id="servicecount" name="servicecount" type="hidden"
                    <?php

                    if (isset ( $_request ['edit'] )) {
                        echo "value='$data[services_count]'";
                    } else {
                        echo 'value="0"';
                    }
                    ?>>
    ...
    <tr>
        <table class="service" width="100%" id="service"
            ...
            <tr>
                ...
                <th width="50%" style="text-align: center">Duty Slip No</th>
    ...
    <div class="form-group">
        <div class="col-sm-offset-2 col-sm-10">
            ...
            if (isset ( $_request ['edit'] )) {
                echo '<button type="submit" class="btn btn-warning" name="EditInvoice">Update Invoice Details</button>';
            } else {
                echo '<button type="submit" class="btn btn-success" name="addInvoice">Add Invoice </button>';
            }
            ?>
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            foreach ( $data as $data ) {
                ...
                echo "<td><a href='/operator/GenerateInvoice?edit=$data[package_id]'> <span class='glyphicon glyphicon-pencil text-success'></span>Edit</a> | <span class='glyphicon glyphicon-trash text-danger'></span> <a href='/operator/generateinvoice?delete=$data[package_id]' >Delete</a> </td></td>";
                echo "</tr>";
                $i ++;
            }
            ?>
        ...
        <a type="button" class="btn btn-success btn-lg"
           href="./operator/generateinvoice?add"> <span
                class="glyphicon glyphicon-plus"></span> Add New Invoice
        </a>
    </div>
    <!--/show table here -->
